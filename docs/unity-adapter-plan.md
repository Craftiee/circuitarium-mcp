...
Godot profile. The read-only controlled-save comparison is implemented. The
next major foundation is a lossless no-op representation, not a general editor.
...
forms, and fingerprint unmodeled ordered XML. It still does not retain a
write-ready copy of every unknown subtree, namespace spelling, or formatting
detail. Re-serializing an arbitrary decoded design could therefore discard data
that CRUMB still needs.
...
independently authored fixture or controlled baseline
      ...
      byte identity / modeled change / coverage
        ...
        evidence record       writer acceptance gate
              |                      |
        catalog update      new-file-only build support
```
...
## Stage 3: lossless edit foundation — planned

Before general editing, add a syntax-preserving representation that can:

- retain unknown root fields and component payload subtrees;
- retain namespace meaning and element order;
- retain scalar lexical forms;
- produce a byte-identical no-op result where possible; and
- explain every unavoidable representation change.

Editing should then be split into two tools:
...
| Publish | Synthetic fixture, tests, provenance note, and catalog write claim |
...
After lossless editing exists:

- add conservative saved-state connectivity for slide and DIP switches;
- add static electrical-rule diagnostics without claiming simulation;
