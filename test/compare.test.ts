...
test("namespace aliases are representation-neutral while invalid schema bindings are inconclusive", () => {
  const baseline = generateFixture("breadboard-resistor");
  ...
  const wrongXsdComparison = compareCru(baseline, wrongXsdCandidate);
  assert.equal(wrongXsdComparison.equivalence.coverage, "partial");
  assert.equal(wrongXsdComparison.equivalence.assessment, "inconclusive");
  assert.equal(
    wrongXsdComparison.profileAssessment.candidate.status,
    "inconclusive",
  );
});
...
test("dual boolean element spellings cannot hide an ignored branch", () => {
