...
test("numeric, int32, boolean, and base64 lexical forms are decoded strictly", () => {
  ...
  assert.throws(
    () => decodeCru(malformedTie),
    /ArrayOfTiePointID payload/,
  );
  assert.throws(
  ...
  for (const entry of cases) {
    const xml = appendBreadboardPayload(breadboard, entry.payload);
    if (entry.kind === "unknown") {
      assert.throws(() => decodeCru(xml), /payload/);
    } else {
      const decoded = decodeCru(xml).components[0]?.values.at(-1);
      const decodedValue =
...
test("malformed known scalar payloads fail atomically instead of being positionally trusted", () => {
  ...
  const validation = validateCru(xml);
  assert.equal(validation.valid, false);
  assert.equal(validation.inspection, undefined);
});
